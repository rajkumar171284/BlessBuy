// BlessBuy - Bulk Add Products to Firestore (INR Products)
// Run this in Firebase Console (Cloud Firestore) to load all products
...
const products = [
  ...
  // Beauty & Personal Care
  {
    ...
    image: "https://images.unsplash.com/photo-1606091839311-d5365f9ff1c5?w=500&h=500&fit=crop",
...
// This runs directly in your application (not Firebase Console)
async function bulkAddProducts() {
  try {
    // Import Firebase functions
    const { collection, addDoc, getFirestore } = await import('https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js');
    ...
    console.log(`\n📊 Results: ${successCount} products added, ${errorCount} errors`);
  } catch (error) {
    console.error('Error initializing Firebase:', error);
  }
}

// Alternative: Direct script for Angular application console
// Copy this to browser console while logged into your BlessBuy app
async function bulkAddProductsFromApp() {
  try {
...
// Run the function (use the app version if available)
if (typeof bulkAddProductsFromApp !== 'undefined') {
  bulkAddProductsFromApp();
} else {
  bulkAddProducts();
}
